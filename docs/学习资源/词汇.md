...
* krass （“厉害了，不得了，夸张”的正面意思，也有反向的贬义）

  * grass（狂野，可怕）
  * die Feige（无花果），Ohrfeige（耳光），feige（怂）
* sautieren（炒（菜））

  * das Resultat（回+跳→结果）
  * frittieren（油炸），Pommes frites（果（土豆）+油炸→炸薯条）
  * panieren（面包→）给食物裹上面粉（为了之后炸）
  * reduzieren（大火收汁）
  * blanchieren 焯水，der Montblanc（山+白→勃朗峰）
  * ziselieren（切→）改花刀
  * flambieren（火燎），die Flamme（火焰）
* der Mönchvc

  * die Monogamie（一夫一妻制）
  * monoton（单调），Monolog（独白）
  * das Monopol（垄断），der Monarch（君主）
* die Kanone（加农炮）

  * mit Kanonen auf/nach Spatzen schießen 加农炮打麻雀
  * das Kanonenfutter（炮灰）
  * der Kanon（准则）

    * unter aller Kanone 在所有的标准之下 - 差到爆表

  * die Kanne（壶），der Kanister（油桶，汽油桶等）
  * die Kanüle（针头，die Nadel 针），der Kanal（运河，频道）

* die Parade（阅兵）

  * das Präparat（标本），parat（准备好的）
  * der Fotoapparat（照相机）
  * das Imperium（帝国）
* die Geborgenheit（安全）

  * bergen（保护，救援）
  * der Berg（山），die Burg（城堡）
  * borgen（借）
  * die Herberge（招待所），das Heer（陆军）
* mahlen（磨，研磨）

  * die Mühle（磨，磨坊），der Müller（磨坊主）
  * der Müll（垃圾）
  * das Mehl（面粉）
  * das Moll（小调，本来是“软音”的意思）
  ...
  * schmelzen（融化，熔化）
    ...
    * die Milz（脾）
    * das Malz（麦芽）
    * das Email（釉质），die E-Mail（邮件）

  * die Milbe（螨虫）
  * der Maulwurf（鼹鼠）
  * Gottes Mühlen mahlen langsam aber fein（天网恢恢疏而不漏，虽迟但到）
